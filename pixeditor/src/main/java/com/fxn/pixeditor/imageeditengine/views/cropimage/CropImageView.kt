...
class CropImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
     ...
     * To set square/circle2 crop shape set aspect ratio to 1:1.
     */
    ...
    val cropWindowRect: RectF?
        get() = mCropOverlayView?.cropWindowRect
    ...
    init {
        ...
        mCropOverlayView!!.mCropWindowChangeListener = object : CropOverlayView.CropWindowChangeListener {
        ...
        /* mCropOverlayView!!.setCropWindowChangeListener(object ::CropOverlayView.CropWindowChangeListener{})
    ...
    public override fun onSaveInstanceState(): Parcelable? {
        ...
        if (imageUri != null && mBitmap != null) {
            ...
            var pair = Pair(key, WeakReference(mBitmap!!))
            BitmapUtils.mStateBitmap = pair
    ...
    private fun applyImageMatrix(width: Float, height: Float, center: Boolean, animate: Boolean) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            if (center) {
                ...
                // was moved outside
                mZoomOffsetX =
                    Math.min(Math.max(mZoomOffsetX * scaleX, -cropRect.left), -cropRect.right + width) / scaleX
                mZoomOffsetY =
                    Math.min(Math.max(mZoomOffsetY * scaleY, -cropRect.top), -cropRect.bottom + height) / scaleY
            }
